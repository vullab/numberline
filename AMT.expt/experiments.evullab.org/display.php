...
if($curtrial <= EXPT_p1n){
	$phase = 1;
} elseif($curtrial <= (EXPT_p1n + EXPT_p2n)){
	$phase = 2;
} elseif($curtrial <= (EXPT_p1n + EXPT_p2n + EXPT_p3n)){
	$phase = 3;
} else {
	$phase = 0;
}
...
<html>
	<head><title>My test</title>
		...
		function start(){
			...
			setTimeout("ctx.clearRect(0,0,WIDTH,HEIGHT);",<?=EXPT_showtime?>);
		}
		...
		function submitme(){
			...
			if(firstsub == 1){
				...
				$("#rt").val((new Date()).getTime()-starttime.getTime());
				thisscore = Math.round(Math.max(0,10-Math.pow(log10($("#response").val()/<?=($curfeedback>0)?$curfeedback:$curshow?>),2)/<?=EXPT_accslope?>));
	...
	<div align="center">
		...
		<span id="feedback" name="feedback" style='font-weight:bold;'></span><br>
		<span id="scoreshow" name="scoreshow" style='font-weight:bold;'></span><br>
