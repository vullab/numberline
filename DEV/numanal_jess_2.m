...
for fi = [1:length(files)]
    ...
    Z = textscan(curfile,'%f %f %f %f %s %f ', 'delimiter', ',');
    q = cell2mat(Z([1:4,6]));
    fclose(curfile);
    for i = [1:length(q)]
        ok(i) = strcmp(Z{5}{i}, 'Uncalibrated');
    end
    n(fi) = sum(ok);
    if(sum(ok)>=30)
        nsubs = nsubs+1;
...
for i = [1:length(z)]
    subplot(7,7,i);
    idxuse = 1:length(z{i}); % (sometimes I use part of the trials to illustrated split half differences)
