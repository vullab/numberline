...
subcuts <- c(0, 25, 125, 225, 325, 425, 525, 625, max(dat$trial))
...
ylims = xlims
ggplot(dat, aes(x=num_dots, y=answer))+
  geom_point(colour="blue", size=2, alpha=0.25)+
  geom_abline(position="identity")+
  mylogx(xlims)+
  mylogy(ylims)+
  xlab("Number presented")+
...
Calibration when varying different things.
```{r  fig.width=16, fig.height=8}
ylims = xlims
...
ggplot(dat, aes(x=num_dots, y=answer))+
    ...
    xlab("Number presented")+
    ylab("Number reported")+
    #mytheme+ # TODO erikb commented this out
