...
for fi = [1:length(files)]
    ...
    zip = textscan(curfile,'%f %f %f %f %f %f %f %f %f ', 'delimiter', ',');
    q1 = cell2mat(zip(1:4));
    q2 = cell2mat(zip(5:7));
    fclose(curfile);
    if(length(q1)==750)
...
%% calculate correlation between answers and responses as a function of block for each subject

for i = [1:nsubs]
    for k = [1:5]
        tidx = [1:150]+(k-1)*150;
        q = corr(log10(z{i}(tidx,nidx)+1), log10(z{i}(tidx,aidx)+1));
        if(isnan(q)) 
            [i k]
        end
       ca(i,k) = q;
...
figure();
for k = [1:length(subsets)]
    varnorm{k} = vk{k};%./(repmat(V1, [length(useblocks)-1 1]).*repmat([1:(length(useblocks)-1)]', [1 length(v1{k})]));
    errorbar([1:(length(useblocks)-1)], mean(varnorm{k},2), std(varnorm{k},[],2)./sqrt(length(v1{k})), colors{k})
    hold on;
end
...
figure();
for k = [1:length(subsets)]
    curv = log10(varnorm{k}./varnorm{2});
    errorbar([1:(length(useblocks)-1)], mean(curv,2), std(curv,[],2)./sqrt(length(v1{k})), colors{k})
    hold on;
end
xlabel('k','FontSize',16)
ylabel('var_k / (var_1 * k)','FontSize',16);
...
figure();
errorbar([1:75], mean(S(:,grp1),2), std(S(:,grp1),[],2)./sqrt(length(grp1)), 'b-')
hold on
errorbar([1:75], mean(S(:,grp2),2), std(S(:,grp2),[],2)./sqrt(length(grp2)), 'r-')
axis tight
ys = ylim();
for i = [25.5:25:50.5]
    plot([i i], ys, 'k-')
