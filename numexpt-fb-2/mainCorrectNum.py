...
def runExperiment(f):
	...
	feedback = False
	parts = 3
	a_choices = [1-slope,1-slope/2,1, 1+slope/2,1+slope]
	for part in range(0,parts):
		...
		if feedback == False:
			...
			for configuration in configurations:
				...
				fbn = 0
				f.write('%d,%d,%d,%d,%f,%d,%d,%.2f\n' % (part+1,len(configuration),answer,feedback,a,fbn, points,resp_time))
		if feedback == True:
			...
			for configuration in configurations:
				...
				displayText ('Guess how many dots were on the screen', 200, 50, black)
				answer = inputValue()
				truen = len(configuration)
				resp_time = time.time() - start_time
				if len(configuration) <= thresh:
					fbn = truen
					points = max(0, round(10 - 10*(abs(math.log10(answer+1) - math.log10(fbn+1)))))
				if len(configuration) > thresh:
					fbn = round(10**(a*(math.log10(truen)-math.log10(thresh))+math.log10(thresh)))
					points = max(0, round(10 - 10*(abs(math.log10(answer+1) - math.log10(fbn+1)))))
				print points
				tot_points += points
				instruct(drawer, ['You scored %d points for a total of %d!' % (points, tot_points),
								  'The correct answer was: %d' % fbn,
...
instruct(drawer, ['Welcome to the Experiment!',
				  ...
				  'There are going to be several parts. ',
				  'During some parts you will see how many points you got and the answer.',
				  'During other parts, you still get points, but will not see your score',
				  '',
				  'The point system is based on the accuracy of your guesses so try your best.',
				  '',
