...
for(i in 1:length(SUBS)){
  ...
  fit = NULL
  while(is.null(fits)){
#     try(fit <- summary(mle(nLL, start=list(a=rnorm(1, subset(bipower.fits, bipower.fits$subject==subject[i])$b, 0.001), #runif(1, -2, 0)
#                                            b=runif(-1, 1, 0)))), TRUE)
    try(fit <- summary(mle(nLL, start=list(a=rnorm(1, subset(bipower.fits, bipower.fits$subject==subject[i])$b, 0.1)))), TRUE)
    iter = iter+1
  ...
  names(fits) <- c("subject", "logL", "n", "a", "sa")
  if(! (fits["logL"] == -999)){
    resp <- thismurp(fits["a"])
...
save(file='fake4data.Rdata', fake, tfits)